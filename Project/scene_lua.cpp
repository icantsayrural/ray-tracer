...
int gr_particles_cmd(lua_State* L) {
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode( name, new Particles() );
...
int gr_render_cmd(lua_State* L)
{
  ...
  ParticleSystem *ps = new ParticleSystem(T > 1);
  std::string s(filename);

  for (int t = 0; t < T; t++) {
    Image im(width, height);

    Project_Render(root->node, im, eye, view, up, fov, ambient, lights, ps);
    im.savePng(s + "_" + std::to_string(t) + ".png");
...
int gr_material_cmd(lua_State* L)
{
  ...
  const char* texture_fname = luaL_checkstring(L, 7);
  const char* bump_fname = luaL_checkstring(L, 8);
...
bool run_lua(const std::string& filename, int t)
{
  T = t;
